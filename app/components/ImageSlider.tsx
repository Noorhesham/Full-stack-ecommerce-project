...
const ImageSlider = ({
  urls,
  productId,
  paginationImage = false,stock
}: {
  urls: string[];
  productId?: string;
  paginationImage?: boolean;stock?:any
}) => {
  ...
  return (
    <div className="group relative bg-zinc-100 aspect-square overflow-hidden rounded-xl">
      <div className="absolute z-10 inset-0 opacity-0 group-hover:opacity-100 transition-all">
        {productId && (
          <button className="opacity-0 group-hover:opacity-100 transition top-3 right-4 absolute">
            <AddToCart stock={stock} productId={productId} />
          </button>
        )}
        <ZoomImage
          btn={
            <button className="opacity-0 group-hover:opacity-100 transition top-3 left-4 absolute">
              <MdOutlineZoomInMap />
            </button>
          }
          urls={urls}
